...
public class ServletMain extends HttpServlet {
	...
	private static void handleRequest6(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			(new ProfileParserV2.AccountManager()).runTest(request, response);
			(new ProfileParserV2.ContextParserBase()).runTest(request, response);
			(new ProfileParserV2.AccountValidatorHelper()).runTest(request, response);
			(new ProfileParserV2.RequestAggregatorBase()).runTest(request, response);
			(new ProfileParserV2.AuditControllerV2()).runTest(request, response);
			(new ProfileParserV2.ReportFactoryUtil()).runTest(request, response);
			(new ProfileParserV2.ScheduleRouterCore()).runTest(request, response);
			(new ProfileParserV2.InternalManager()).runTest(request, response);
			(new ProfileParserV2.InternalProvider()).runTest(request, response);
			(new ProfileParserV2.InternalAggregator()).runTest(request, response);
			(new ProfileParserV2.InternalValidator()).runTest(request, response);
			(new ProfileParserV2.InternalEmitter()).runTest(request, response);
			(new ProfileParserV2.InternalRepository()).runTest(request, response);
			(new ProfileParserV2.InternalRouter()).runTest(request, response);
			(new ProfileParserV2.InternalLoader()).runTest(request, response);
			(new ProfileParserV2.InternalValidator()).runTest(request, response);
			(new ProfileParserV2.InternalFactory()).runTest(request, response);
