package com.foundation.request.core;
...
public class ServletMain extends HttpServlet {
	...
	private static void handleRequest5(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			(new ReportProviderCore.AccountManagerBase()).runTest(request, response);
			(new ReportProviderCore.EventValidatorV2()).runTest(request, response);
			(new ReportProviderCore.ProfileBuilderUtil()).runTest(request, response);
			(new ReportProviderCore.TransactionConverterInternal()).runTest(request, response);
			(new ReportProviderCore.RecordServiceInternal()).runTest(request, response);
			(new ReportProviderCore.RequestManagerHelper()).runTest(request, response);
			(new ReportProviderCore.EventFactoryUtil()).runTest(request, response);
			(new ReportProviderCore.InternalParser()).runTest(request, response);
			(new ReportProviderCore.InternalValidator()).runTest(request, response);
			(new ReportProviderCore.InternalBuilder()).runTest(request, response);
			(new ReportProviderCore.InternalController()).runTest(request, response);
			(new ReportProviderCore.InternalConverter()).runTest(request, response);
			(new ReportProviderCore.InternalRepository()).runTest(request, response);
			(new ReportProviderCore.InternalManager()).runTest(request, response);
			(new ReportProviderCore.InternalProvider()).runTest(request, response);
			(new ReportProviderCore.InternalValidator()).runTest(request, response);
			(new ReportProviderCore.InternalConverter()).runTest(request, response);
